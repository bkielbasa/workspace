...
func (s *Server) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /settings", s.page(s.settingsPage))
	mux.HandleFunc("POST /settings/profile", s.RequireAuth(s.RequireCSRF(s.settingsProfile)))
	mux.HandleFunc("POST /settings/password", s.RequireAuth(s.RequireCSRF(s.settingsPassword)))
	mux.HandleFunc("POST /settings/iphone-profile", s.RequireAuth(s.RequireCSRF(s.settingsIPhoneProfile)))
	mux.HandleFunc("GET /settings/iphone-profile/download", s.settingsIPhoneProfileDownload)
	mux.HandleFunc("POST /settings/app-passwords", s.RequireAuth(s.RequireCSRF(s.settingsAppPasswordCreate)))
	mux.HandleFunc("POST /settings/app-passwords/revoke", s.RequireAuth(s.RequireCSRF(s.settingsAppPasswordRevoke)))
	mux.HandleFunc("POST /settings/invites", s.RequireAuth(s.RequireCSRF(s.settingsInviteCreate)))
	mux.HandleFunc("POST /settings/invites/revoke", s.RequireAuth(s.RequireCSRF(s.settingsInviteDelete)))
	mux.HandleFunc("POST /settings/signatures", s.RequireAuth(s.RequireCSRF(s.settingsSignatureCreateOrUpdate)))
	mux.HandleFunc("POST /settings/signatures/{id}/delete", s.RequireAuth(s.RequireCSRF(s.settingsSignatureDelete)))
	mux.HandleFunc("POST /settings/signatures/{id}/default", s.RequireAuth(s.RequireCSRF(s.settingsSignatureSetDefault)))
	mux.HandleFunc("POST /settings/rules", s.RequireAuth(s.RequireCSRF(s.settingsRuleCreate)))
	mux.HandleFunc("POST /settings/rules/{id}/delete", s.RequireAuth(s.RequireCSRF(s.settingsRuleDelete)))
